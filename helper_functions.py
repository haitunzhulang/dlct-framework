...
def plot_multi_loss(model_name,train_loss_dic,val_loss_dic):
	...
	if (len(train_loss_dic['loss'])>4):
		...
		bx.plot(train_loss_dic['ori'],'b-',linewidth=1.3)
		bx.plot(val_loss_dic['ori'],'r-',linewidth=1.3)
		bx.set_title('Red_1 loss')
		bx.set_ylabel('MSE')
		bx.set_xlabel('epochs')
		bx.legend(['train', 'test'], loc='upper left')
		cx = fig.add_subplot(2,3,3)
		cx.plot(train_loss_dic['red2'],'b-',linewidth=1.3)
		cx.plot(val_loss_dic['red2'],'r-',linewidth=1.3)
		cx.set_title('Red_2 loss')
		cx.set_ylabel('MSE')
		cx.set_xlabel('epochs')
		cx.legend(['train', 'test'], loc='upper left')
		dx = fig.add_subplot(2,3,4)
		dx.plot(train_loss_dic['red4'],'b-',linewidth=1.3)
		dx.plot(val_loss_dic['red4'],'r-',linewidth=1.3)
		dx.set_title('Red_4 loss')
		dx.set_ylabel('MSE')
		dx.set_xlabel('epochs')
		dx.legend(['train', 'test'], loc='upper left')
		ex = fig.add_subplot(2,3,5)
		ex.plot(train_loss_dic['red8'],'b-',linewidth=1.3)
		ex.plot(val_loss_dic['red8'],'r-',linewidth=1.3)
...
# 			K.set_value(model.optimizer.loss,'mse_ct_err')
		...
		densitylist =None
		hist = model.fit(trainImages,[red8, red4, red2, densityMaps],batch_size=batch_size, nb_epoch=nb_epoch_per_record, verbose=1, shuffle=True)
# 		hist=model.fit(trainImages,densityMaps,batch_size=batch_size, nb_epoch=nb_epoch_per_record, verbose=1, shuffle=True)
		...
		score=model.evaluate(valImages,[val_red8, val_red4, val_red2, valDensityMaps],verbose=1,batch_size=int(batch_size))
...
## training with control of whether to compute the mean of absolute error
def train_model(model, X_train, X_val, Y_train, Y_val, nb_epochs=400, nb_epoch_per_record=1, input_shape=(100,100,1), batch_size =256, is_mae = False, lr_max = None):
	import random
	keras.losses.mse_ct_err = mse_ct_err
	if len(X_train.shape)<3:
		X_train = X_train.reshape(X_train.shape + (1,))
